...
job "addsvc-job" {
  ...
  group "addsvc-group" {
    ...
    task "addsvc-task" {
      ...
      # config {
			# 	jar_path    = "local/add.svc-1.0.0.jar"
			# 	jvm_options = ["-Xmx400m", "-Xms128m", "-Dspring.profiles.active=dev"]
      # }
			config {
				command = "java"
				args = ["-Xmx400m", "-Xms128m", "-Dspring.profiles.active=dev", "-jar", "local/add.svc-1.0.0.jar"]
			}   
      ...
      #
      artifact {
				source = "http://localhost/public/libs/add.svc-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
    ...
    task "log-shipper-task" {
      driver = "raw_exec"
			...
			config {
				...
				args = ["-e", "-c", "local/conf/addsvc-filebeat.yml"]
			}   
      artifact {
				source = "http://localhost/public/log-shipper/addsvc-filebeat.yml"
