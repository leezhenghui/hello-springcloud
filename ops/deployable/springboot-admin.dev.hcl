...
job "springboot-admin-job" {
  ...
  group "springboot-admin-group" {
    ...
    task "springboot-admin-task" {
      ...
      # config {
	  #			jar_path    = "local/springboot.admin-1.0.0.jar"
	  #			jvm_options = ["-Xmx800m", "-Xms128m", "-Dspring.profiles.active=dev"]
      # }
      config {
      	command = "java"
      	args = ["-Xmx400m", "-Xms128m", "-Dspring.profiles.active=dev", "-jar", "local/springboot.admin-1.0.0.jar"]
      }
      ...
      #
      artifact {
				source = "http://localhost/public/libs/springboot.admin-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
      ...
      # kill_timeout = "20s"
       ...
       driver = "raw_exec"
      			...
      			config {
      				...
      				args = ["-e", "-c", "local/conf/springboot-admin-filebeat.yml"]
      			}
            artifact {
      				source = "http://localhost/public/log-shipper/springboot-admin-filebeat.yml"
