...
job "subsvc-job" {
  ...
  group "subsvc-group" {
    ...
    task "subsvc-task" {
      ...
      # config {
			# 	jar_path    = "local/sub.svc-1.0.0.jar"
			# 	jvm_options = ["-Xmx400m", "-Xms128m", "-Dspring.profiles.active=dev"]
      # }
			config {
				command = "java"
				args = ["-Xmx400m", "-Xms128m", "-Dspring.profiles.active=dev", "-jar", "local/sub.svc-1.0.0.jar"]
			}   
      ...
      #
      artifact {
				source = "http://localhost/public/libs/sub.svc-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
    ...
    task "log-shipper-task" {
      driver = "raw_exec"
			...
			config {
				...
				args = ["-e", "-c", "local/conf/subsvc-filebeat.yml"]
			}   
      artifact {
				source = "http://localhost/public/log-shipper/subsvc-filebeat.yml"
