...
job "api-gateway-job" {
  ...
  group "api-gateway-group" {
    ...
    task "api-gateway-task" {
      ...
      #
      artifact {
				source = "http://localhost/public/libs/api.gateway-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
      ...
      driver = "raw_exec"
			...
			config {
				...
				args = ["-e", "-c", "local/conf/api-gateway-filebeat.yml"]
			}   
      artifact {
				source = "http://localhost/public/log-shipper/api-gateway-filebeat.yml"
