...
job "calculator-ui-job" {
  ...
  group "calculator-ui-group" {
    ...
    task "calculator-ui-task" {
      ...
      # config {
			# 	jar_path    = "local/calculator.ui-1.0.0.jar"
			# 	jvm_options = ["-Xmx512m", "-Xms128m", "-Dspring.profiles.active=dev"]
      # }
			config {
				command = "java"
				args = ["-Xmx512m", "-Xms128m", "-Dspring.profiles.active=dev", "-XX:-UseGCLogFileRotation", "-XX:GCLogFileSize=20M", "-XX:NumberOfGCLogFiles=1", "-XX:+PrintGCApplicationStoppedTime", "-XX:+PrintSafepointStatistics", "-XX:PrintSafepointStatisticsCount=1", "-Xloggc:${NOMAD_ALLOC_DIR}/logs/calculator-ui-gc.log", "-jar", "local/calculator.ui-1.0.0.jar"]
			}   
      ...
      #
      artifact {
				source = "http://localhost/public/libs/calculator.ui-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
      ...
      service {
				name = "calculator-ui"
        tags = ["spring-boot", "java", "edger-service"]
        port = "http"
        check {
					type = "http"
					port = "http"
    ...
    task "log-shipper-task" {
      driver = "raw_exec"

			config {
				command = "/usr/share/filebeat/bin/filebeat"
				args = ["-e", "-c", "local/conf/calculator-ui-filebeat.yml"]
			}   
      artifact {
				source = "http://localhost/public/log-shipper/calculator-ui-filebeat.yml"
      ...
      service {
				name = "calculator-ui-log-shipper"
        tags = ["log-shipper", "golang"]
      }
