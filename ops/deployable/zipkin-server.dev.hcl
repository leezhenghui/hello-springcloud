...
job "zipkin-server-job" {
  ...
  group "zipkin-server-group" {
    ...
    task "zipkin-server-task" {
      ...
      config {
				jar_path    = "local/zipkin.server-1.0.0.jar"
      ...
      #
      artifact {
				source = "http://localhost/public/libs/zipkin.server-1.0.0.jar"
				destination = "local/"
				options {
					archive = true 
        }
      ...
      driver = "exec"
			...
			config {
				...
				args = ["-e", "-c", "local/conf/zipkin-server-filebeat.yml"]
			}   
      artifact {
				source = "http://localhost/public/log-shipper/zipkin-server-filebeat.yml"
      ...
      service {
				name = "zipkin-server-log-shipper"
